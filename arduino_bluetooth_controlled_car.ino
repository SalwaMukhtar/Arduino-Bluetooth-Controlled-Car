...
void loop()
{
  if (Serial.available() > 0)
  {
   ...
   //Change pin mode only if new command different from previous.
...
void forward()
{
  motor1.setSpeed(255); //Define maxiam velocity
  motor1.run(FORWARD); //Rotate the motor clockwise
  motor2.setSpeed(255); //Define maxiam velocity
  motor2.run(FORWARD); //Rotate the motor clockwise
  motor3.setSpeed(255); //Define maxiam velocity
  motor3.run(FORWARD); //Rotate the motor anti-clockwise
  motor4.setSpeed(255); //Define maxiam velocity
